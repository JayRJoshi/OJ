...
$problem_user_info_query = "SELECT Problem_Id,Problem_Name,Location,total_user_submissions,total_submissions,correct_submissions,incorrect_submissions FROM problem_table";
...
if ($problem_user_info->num_rows > 0) {
    ...
    while($row = $problem_user_info->fetch_assoc()) {
		...
		$tags = "";
		//problem_status_query will give status for particular problem for logged-in user.
		...
		echo "<td>".$i."</td>
		<td><a href=\"editor.php?prob_name=".$row["Problem_Name"]."&prob_id=".$row["Problem_Id"]."&loc=".$row["Location"]."\">".$row["Problem_Name"]."</td>
